...
const executableResolve = (): { entrypoint: string; target?: string } => {
...
const installationTypeResolve = (executableTarget: string | undefined): string => {
...
const requirementsRender = (): string =>
  Object.entries(packageMetadata.engines ?? {})
    .map(([runtime, requirement]) => `${runtime} ${requirement}`)
    .join(", ") || "unavailable"

export const assetsCliVersionMetadataRender = (): string => {
  ...
  const lines = [
    `assets ${packageMetadata.version}`,
