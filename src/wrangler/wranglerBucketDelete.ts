...
export const wranglerBucketDelete = async (
  ...
  const info = await commandInvoke(runner, ["r2", "bucket", "info", input.bucket, "--json"], input.profile, credentials)
  ...
  const deleted = await commandInvoke(
    runner,
    ["r2", "bucket", "delete", input.bucket, "--force"],
    input.profile,
    credentials,
  )
...
async function commandInvoke(
  ...
  try {
    ...
    if (!result.success) return resultErrorCreate("wranglerBucketDelete", "Wrangler command failed")
    return result
  } catch {
    return resultErrorCreate("wranglerBucketDelete", "Wrangler command failed")
  }
}
