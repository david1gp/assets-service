...
export const jobRepositoryPayloadUpdate = (
  db: AssetDatabase,
  input: JobRepositoryPayloadUpdateInput,
  ...
  const update = (transaction: AssetDatabase): Result<Job> => {
    const updated = transaction
      .update(jobTable)
      .set({ payload: parsed.output, updatedAt: now })
      .where(and(eq(jobTable.id, input.jobId), eq(jobTable.status, "running"), eq(jobTable.leaseOwner, input.workerId)))
      .returning()
      .get()
  ...
  if (transactionInput !== undefined) return update(transactionInput)
  return databaseTransactionRun<Job>(db, update, { behavior: "immediate" })
}
