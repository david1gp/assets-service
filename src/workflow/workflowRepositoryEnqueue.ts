...
export const workflowRepositoryEnqueue = (
  ...
  transactionInput === undefined
    ? databaseTransactionRun<WorkflowRepositoryEnqueueResult>(
        db,
        (transaction) => workflowRepositoryEnqueueRun(transaction, input),
        { behavior: "immediate" },
      )
    : workflowRepositoryEnqueueRun(transactionInput, input)

function workflowRepositoryEnqueueRun(
  transaction: AssetDatabase,
  input: WorkflowRepositoryEnqueueInput,
): Result<WorkflowRepositoryEnqueueResult> {
  const op = "workflowRepositoryEnqueue"
  const existingWorkflow = transaction.select().from(workflowTable).where(eq(workflowTable.id, input.workflow.id)).get()
  ...
  for (const job of input.jobs) {
    if (job.workflowId !== input.workflow.id) return resultErrorCreate(op, `Job belongs to another workflow: ${job.id}`)

    const existingJob = transaction.select().from(jobTable).where(eq(jobTable.idempotencyKey, job.idempotencyKey)).get()
