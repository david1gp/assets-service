...
- Select customer projects through a reviewed stable binding allowlist; initially only `Abikur` is selected.
...
- Reconcile the two-organization policy in the server provisioning scripts and deploy in an order that avoids accidental privilege expansion.
...
- [x] 4. Update Zitadel provisioning for the organization policy and provide explicit post-rollout cleanup for the temporary individual Tim grant.
