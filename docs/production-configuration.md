...
Set `ZITADEL_ISSUER`, `ZITADEL_CLIENT_ID`, `ZITADEL_REDIRECT_URI`, `ZITADEL_AUDIENCE`, `ZITADEL_ORGANIZATION_ID`, `ZITADEL_CUSTOMER_ORGANIZATION_ID`, and `ZITADEL_PROJECT_ID`. `ZITADEL_ORGANIZATION_ID` identifies the staff organization and `ZITADEL_CUSTOMER_ORGANIZATION_ID` identifies the customer organization; both are required and must be different. Set `ZITADEL_SERVICE_ACCOUNT_CLIENT_ID` only when service-to-service bearer tokens are needed. Set `ZITADEL_PROJECT_PROVISIONER_SUBJECT_ID` for one exact machine identity authorized for noninteractive project registration. Add exact comma-separated identities with `ZITADEL_PROJECT_PROVISIONER_SUBJECT_IDS`; the plural setting supplements the singular setting. Provisioner authorization remains PAT-only and restricted to configured owner organizations. Uploads still require a matching Zitadel project grant. The doctor checks HTTPS issuer discovery and JWKS reachability, but it does not print credentials.
