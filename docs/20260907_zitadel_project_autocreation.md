...
- Reuse the dedicated provisioner with organization project-creation permission and the existing Assets runtime identity with project-scoped access.
- Use `contentoren-development` for development storage and protected `contentoren.env` for runtime CLI credentials.
...
3. Complete: Luna agent uses commits skill to commit/push intended changes, then deploy.
4. Complete: SSH as `leo` to `leo-server`, inspect `~/projects/contentoren`, and test setup, creation, and upload of all project assets using the assets skill.
